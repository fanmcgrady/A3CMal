...
def custom_cross_validation(data, class_labels, classifier_name = 'xgb', n_splits = 5, hash_codes = [],
                            print_mc_samples = False, bagging = False):
    kfold = StratifiedKFold(n_splits = n_splits, shuffle = True, random_state = 1000)
    ...
    for train_index, test_index in kfold.split(data, class_labels):
        fold_counter += 1
...
def train_test_validation(train_data, train_id, test_data, features_name = [], classifier_name = 'xgb',
    ...
    with gzip.open(submission_file, 'w') as f:
        ...
        fw.writerow(header)
        for t, (Id, pred) in enumerate(zip(test_hash, pred_prob.tolist())):
            fw.writerow([Id]+pred)
            if(t+1)%1000==0:
...
def xgboost_cla(train_data, train_id, test_data, seed = None):
    ...
    #将模型保存到本地
    pickle.dump(clf, open(os.path.join(MODEL_PATH, 'novel_model.dat'), 'wb'))
